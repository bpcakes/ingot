...
use super::item::{insert_revision_query, update_item_query};
...
impl Database {
    pub async fn apply_revision_lane_teardown(
        ...
        apply_revision_lane_teardown_in_tx(&mut tx, &mutation).await?;
        tx.commit().await.map_err(db_err)?;
        Ok(())
    }

    pub async fn apply_revision_lane_teardown_and_create_revision(
        &self,
        mutation: RevisionLaneTeardownMutation,
...
async fn apply_revision_lane_teardown_in_tx(
    ...
    for cancellation in &mutation.job_cancellations {
        ...
        if result.rows_affected() != 1 {
            if item_revision_is_stale(tx, params.item_id, params.expected_item_revision_id).await? {
